...
export default function SignUp() {
  ...
  const ok = passwordOk(checks) && password === confirm
  ...
  const Rule = ({ pass, text }: { pass: boolean; text: string }) =>
    <div className={pass ? 'success' : 'helper'}>• {text}</div>
  ...
  return (
    <div className="section" style={{ maxWidth: 520, margin: '40px auto' }}>
      ...
      <form onSubmit={onSubmit} className="grid" style={{ gap: 12 }}>
        {/* @ts-ignore */}
        <md-outlined-text-field label="Full name" value={name} oninput={(e: any) => setName(e.target.value)} required />
        {/* @ts-ignore */}
        <md-outlined-text-field type="email" label="Email" value={email} oninput={(e: any) => setEmail(e.target.value)} required />
        {/* @ts-ignore */}
        <md-outlined-text-field type="password" label="Password" value={password} oninput={(e: any) => setPassword(e.target.value)} required />
        {/* @ts-ignore */}
        <md-outlined-text-field type="password" label="Confirm password" value={confirm} oninput={(e: any) => setConfirm(e.target.value)} required />
        ...
        {/* @ts-ignore */}
        <md-filled-button type="submit" disabled={!ok}>Create account</md-filled-button>
